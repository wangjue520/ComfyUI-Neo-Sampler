# Vendored from Forge Neo: modules_forge/packages/k_diffusion/sampling.py (neo branch, 2026-09-18)
# (which itself originates from https://github.com/comfyanonymous/ComfyUI/blob/v0.3.75/comfy/k_diffusion/sampling.py)
# Only the import lines were changed. NOTE: the module-level name `torch` is swapped at runtime
# for a TorchHijack object (exactly like Forge does) so that torch.randn_like draws from Neo's ImageRNG.
...
from . import k_utils as utils


def set_model_options_post_cfg_function(model_options, post_cfg_function, disable_cfg1_optimization=False):
    # identical to Forge backend/patcher/base.py
...
# ---- Forge Neo modules/sd_schedulers.py overrides k_diffusion.sampling.to_d at import time ----
def _forge_to_d(x, sigma, denoised):
    """Converts a denoiser output to a Karras ODE derivative"""
